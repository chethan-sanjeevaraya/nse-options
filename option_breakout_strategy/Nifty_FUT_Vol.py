...
def run_breakout_strategy_dynamic():
    ...
    nifty_fut = get_nearest_nifty_fut()
    if not nifty_fut:
        logging.error("[❌] Cannot proceed without NIFTY Futures contract.")
        return

    nifty_fut_token = nifty_fut["instrument_token"]
    fut_symbol = nifty_fut["tradingsymbol"]
    fut_expiry = nifty_fut["expiry"]
    ...
    while datetime.now() < market_end:
        ...
        try:
            ...
            # Fetch 5-min candle from nearest NIFTY Futures (not spot index)
            candle = fetch_5min_candle(nifty_fut_token, prev_start, prev_end)

            prev_candle_high = candle["high"]
            prev_candle_low = candle["low"]
            prev_candle_volume = candle["volume"]

            logging.info(
            f"📊 Previous Candle {prev_start.time()} - {prev_end.time()} | "
            f"FUT={fut_symbol} | Expiry={fut_expiry} | "
            f"High={prev_candle_high:.2f} | Low={prev_candle_low:.2f} | "
            f"Close={candle['close']:.2f} | Vol={prev_candle_volume}"
            )
        ...
        while datetime.now() < candle_end_time:
            ...
            if ltp > breakout_above:
                logging.info(f"[🔼] Breakout above previous high at LTP={ltp:.2f} | FUT={fut_symbol}")
                place_atm_sell_order(ltp, "PE")
                return

            elif ltp < breakout_below:
                logging.info(f"[🔽] Breakdown below previous low at LTP={ltp:.2f} | FUT={fut_symbol}")
        ...
        logging.info(
            ...
            f"| Breakout Above={breakout_above:.2f} | Below={breakout_below:.2f} | Vol={prev_candle_volume}"
        )
