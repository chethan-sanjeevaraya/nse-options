...
def exit_trade(symbol):
    """
    Exit trade by buying back the same option symbol.
    """
    try:
        order_id = placeMarketOrder({
            "exchange": "NFO",
            ...
            "variety": "regular",
            "product": "MIS",
            "quantity": 75  # TODO: replace with correct lot size lookup
        })
        logging.info(f"Exit order placed for {symbol} | Order ID: {order_id}")
        return True
    except Exception as e:
        logging.error(f"Error while exiting trade for {symbol}: {e}")
        return False
...
def run_consecutive_candle_strategy():
    """
    - Watch 5-min candles on NIFTY futures/spot (via fetch_5min_candle).
    - Entry when: 3 consecutive candles (all RED then GREEN, or all GREEN then RED),
      with the 4th candle's volume < 3rd candle's volume. Entry price = 4th close (underlying).
    - Place SHORT option at-the-money (CE for 3 RED→GREEN, PE for 3 GREEN→RED).
    - SL/TP computed on underlying from last 3 candles (1:2 RR).
    - Monitor ticks via underlying LTP; exit the option at market on SL/TP hit.
    - Log once per candle close; keep entries/exits responsive to ticks.
    - Allow a second entry only if the first trade’s SL is hit (at most 2 entries/day).
    """
    ...
    logging.info(f"Starting 3-consecutive candle reversal strategy (vol filter, 2nd entry only if SL) from {CANDLE_START} to {CANDLE_END}")
    ...
    while datetime.now() < market_end:
        ...
        try:
            ...
            if (not in_trade) and (entries_taken < 2):
                if len(candle_types) == 4:
                    first_three = candle_types[:3]
                    fourth_type = candle_types[3]

                    # Volume filter: 4th vol < 3rd vol
                    vol_ok = volumes[3] < volumes[2]

                    # 3 RED then a GREEN → short CE
                    if all(ct == "RED" for ct in first_three) and fourth_type == "GREEN" and vol_ok:
                        # only allow second entry if SL was hit
                        if entries_taken == 0 or (entries_taken == 1 and second_entry_unlocked):
                            entry_underlying = closes[3]  # 4th close
                            # compute SL/TP on UNDERLYING using last 3 candles
                            sl_u, tp_u = calculate_sl_tp(raw_candles[-3:], entry_underlying, "BUY_CE", buffer_pct=0.05)
                            # For BUY_CE (bullish underlying), our strategy SHORTS CE (bearish on option price)
                            # tp should be BELOW entry; sl ABOVE entry. calculate_sl_tp returns exactly that for BUY_CE.
                            sl_underlying, tp_underlying = sl_u, tp_u
                            option_type = "CE"

                            # place the option sell at ATM of underlying
                            _, option_symbol = place_atm_sell_order(entry_underlying, option_type)
                            if option_symbol:
                                option_entry_price = get_ltp(f"NFO:{option_symbol}")
                                # get lot size from chain to use later for PnL math and exit qty
                                # (reusing option_chain from your helpers)
                                ...
                                logging.info(f"[🔻] Short {option_type}: {option_symbol} | Entry(U)={entry_underlying:.2f} | SL(U)={sl_underlying:.2f} | TP(U)={tp_underlying:.2f}")

                    # 3 GREEN then a RED → short PE
                    if all(ct == "GREEN" for ct in first_three) and fourth_type == "RED" and vol_ok:
                        if entries_taken == 0 or (entries_taken == 1 and second_entry_unlocked):
                            entry_underlying = closes[3]
