...
# ---------------- CANDLE HELPERS ----------------
def fetch_5min_candle(kite, token: int, start: datetime, end: datetime) -> Optional[Dict[str, Any]]:
    """
    Fetch OHLC for a single 5-min candle.
    Returns dict: {open, high, low, close, volume, start, end}
    """
    try:
        ...
        return {
            ...
            "volume": c.get("volume", 0), "start": c["date"], "end": end
        }
    except Exception as e:
        logging.error("fetch_5min_candle failed: %s", e)
        return None
